...
function clickContinuar() {
  const botonContinuar = document.querySelector(
    "#yDmH0d > div.VfPpkd-Sx9Kwc.VfPpkd-Sx9Kwc-OWXEXe-n9oEIb.VfPpkd-Sx9Kwc-OWXEXe-n9oEIb-OWXEXe-diJVc.cC1eCc.UDxLd.PzCPDd.e0Cfsd.u8pvgb.VfPpkd-Sx9Kwc-OWXEXe-FNFY6c > div.VfPpkd-wzTsW > div > div.VfPpkd-cnG4Wd > div > div > div:nth-child(4) > div > button"
  );
  if (botonContinuar) {
...
function clickSoyEstudiante() {
  ...
  if (!boton) {
    console.warn(
      'Se encontró el contenedor data-role="3", pero no un botón en su interior.'
    );
    return;
  }
...
function clickMenuPerfil() {
  ...
  const menuPerfil = document.querySelector(
    "#gb > div > div.gb_Re > div.gb_z.gb_dd.gb_Nf.gb_0 > div > a"
  );
...
function clickSalirPerfil() {
  ...
  const botonSalir = document
    .querySelector("#yDmH0d > c-wiz > div > div > div > div")
...
function clickOtraCuenta() {
  ...
  const botonOtraCuenta = document
    .querySelector("#yDmH0d > c-wiz")
    .querySelector("ul > li:last-child").previousSibling.children[0];
...
// Apretar botón "ENtendido" del modal de aviso para continuar
function clickModalEntendido() {
  // 1. Buscar el botón "Entendido" en el modal
  const botonEntendido = document.querySelector(
    "#yDmH0d > div.VfPpkd-Sx9Kwc.cC1eCc.UDxLd.PzCPDd.pCXog.VfPpkd-Sx9Kwc-OWXEXe-FNFY6c > div.VfPpkd-wzTsW > div > div.VfPpkd-T0kwCb > div > div > button"
  );

  if (!botonEntendido) {
    console.error('No se encontró el botón "Entendido" en el modal');
    return;
  }

  // 2. Simular clic en el botón
  botonEntendido.click();
  console.log('Botón "Entendido" del modal presionado');
}

// Apretar botón "Unirme" para acceder al aula virtual
function clickUnirme() {
  // 1. Buscar el botón "Unirme"
  let botonUnirme = document.querySelector("#ow13 > div > ol > li").querySelector('button[aria-label*="Unirse a"]')

  if(!botonUnirme) botonUnirme = document.querySelector("#yDmH0d > c-wiz").querySelector('button[aria-label*="Unirse a"]');

  if (!botonUnirme) {
    console.error('No se encontró el botón "Unirme"');
    return;
  }

  // 2. Simular clic en el botón
  botonUnirme.click();
  console.log('Botón "Unirme" presionado');
}
